...
export class FastApiClient {
  ...
  static async generateWeeklyDigest(): Promise<WeeklyDigestResponse> {
    try {
      const res = await fetch(`${FASTAPI_BASE_URL}/api/v1/digest/generate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          engineer_name: 'Backend & MLOps Engineer',
          week_number: 30,
          year: 2026,
          completed_mits_count: 18,
          deep_work_hours: 14.5,
          commits_pushed: 42,
          overall_life_os_score: 88,
          key_wins: [
      ...
      return {
        markdown_report: '# Weekly Digest\nGenerated by HIM OS Backend.',
        summary_sentence: 'Week 30 completed with 18 MITs and 14.5 hrs Deep Work.',
